...
import { getRateLimiter, recordRateLimitHit } from './lib/redis';
...
export async function middleware(request: NextRequest) {
  ...
  // Initialize Supabase client for traffic analytics
  const analyticsClient = createClient(request);
  ...
  try {
    ...
    if (API_ROUTES.test(request.nextUrl.pathname)) {
      ...
      recordTraffic(analyticsClient, request, region, requestStart).catch(err => {
